...
import xyz.fnplus.echo.WiFiDirectActivity;
...
import xyz.fnplus.echo.router.Receiver;
...
public class WiFiBroadcastReceiver extends BroadcastReceiver {
  ...
  public WiFiBroadcastReceiver(WifiManager wifiManager, WiFiDirectActivity activity,
  ...
  public void onReceive(Context context, Intent intent) {
    ...
    if (WifiManager.SCAN_RESULTS_AVAILABLE_ACTION.equals(action)) {
      ...
      if (wfdSsid != null && !this.isWifiConnected) {
        ...
        Toast.makeText(activity, "Found no WiFi direct network to connect to using wlan0",
  ...
  private void changeState(DetailedState aState, Context context) {
    if (aState == DetailedState.SCANNING) {
      ...
      Log.d("WiFiBroadcastReceiver",
          "	ip=" + this.parseIpAddress(wifiManager.getConnectionInfo().getIpAddress()));
      Log.d("WiFiBroadcastReceiver", "	ssid=" + wifiManager.getConnectionInfo().getSSID());
      Log.d("WiFiBroadcastReceiver",
          "	dhcpGtw=" + this.parseIpAddress(wifiManager.getDhcpInfo().gateway));
      ...
      Log.d("WiFiBroadcastReceiver",
          "	netmask=" + this.parseIpAddress(wifiManager.getDhcpInfo().netmask));

      MeshNetworkManager.setSelf(
          new AllEncompassingP2PClient(wifiManager.getConnectionInfo().getMacAddress(),
              Configuration.GO_IP, wifiManager.getConnectionInfo().getMacAddress(),
