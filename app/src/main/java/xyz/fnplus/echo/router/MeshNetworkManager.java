...
public class MeshNetworkManager {
   ...
   * Your routing table
   */
  public static ConcurrentHashMap<String, AllEncompassingP2PClient> routingTable =
      new ConcurrentHashMap<String, AllEncompassingP2PClient>();
    ...
    if (self.getGroupOwnerMac() == self.getMac()) {
      if (self.getGroupOwnerMac() != c.getGroupOwnerMac() && go.getIsDirectLink()) {
        ...
        return c.getIp();
          ...
          && self.getGroupOwnerMac() != c.getGroupOwnerMac()
          && !go.getIsDirectLink()) {
